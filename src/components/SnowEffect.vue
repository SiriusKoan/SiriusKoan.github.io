  ...
  methods: {
    snow() {
      for (let i = 0; i < this.$refs.flake.length; i++) {
        this.$refs.flake[i].style.left =
          Math.max(0.05, Math.min(Math.random(), 0.95)) * window.innerWidth +
          "px";
        ...
        this.$refs.flake[i].style.animationDuration =
          Math.max(15, Math.random() * 40) + "s";
